...
export default function OrganizerLoginViaEmail() {
    ...
    if (!result.success) {
      ...
      result.error.issues.forEach((issue) => {
        if (issue.path[0] === 'email') {
          ...
          // Combine password errors if multiple exist
          formattedErrors.password = formattedErrors.password 
            ? `${formattedErrors.password}, ${issue.message.toLowerCase()}`
            : issue.message;
        }
      });
  ...
  return (
    <div className="w-full min-h-screen h-auto flex flex-col lg:flex-row">
      ...
      <div className={`w-full lg:w-1/2 min-w-0 h-full min-h-screen flex flex-col px-6 lg:px-12 ${dynamicGap} pt-6 lg:pt-28`}>
        ...
        <div className={`w-full h-auto flex flex-col ${dynamicGap}`}>
          ...
          <div className={`flex flex-col w-full h-auto ${dynamicGap}`}>
            <div className={`flex flex-col w-full h-auto gap-2`}>
              ...
              <p className="w-full text-lg lg:text-2xl font-arialrounded font-normal leading-none tracking-normal text-black">
                Log In To Your Existing Account By Email
              </p>
            </div>

            {/* Email Input */}
            <div className={`flex flex-col w-full h-auto pb-6 ${dynamicGap}`}>
              <div className="relative w-full h-12 lg:h-14 font-arialrounded rounded-xl gap-2 bg-[#F2F2F2]">
                <input
                  type="email"
                  id="email"
                  value={email}
              ...
              <div className="relative w-full h-12 lg:h-14 font-arialrounded rounded-xl gap-2 bg-[#F2F2F2]">
                <input
                  ...
                  placeholder=" "
                  className={`peer w-full h-12 lg:h-14 px-4 lg:px-6 pt-5 lg:pt-6 pb-2 lg:pb-3 text-sm lg:text-base font-normal rounded-xl leading-none tracking-wider outline-none focus:outline-none focus:ring-0 ${
                    errors.password ? 'bg-red-100 text-red-700' : 'bg-[#F2F2F2] text-gray-800'
                  }`}
                />
                <label
                  htmlFor="password"
                  className={`absolute left-4 lg:left-6 font-arialrounded font-normal pointer-events-none transition-all duration-200 ease-in-out ${
                    password ? 'top-1 text-[10px] lg:text-xs' : 'top-3 lg:top-4 text-sm lg:text-base'
                  } peer-focus:top-1 peer-focus:text-[10px] lg:peer-focus:text-xs ${
                    errors.password ? 'text-red-500' : 'text-[#737373]'
                  }`}
                >
                  Password
                </label>
                {errors.password && (
                  <p className="text-red-500 text-xs mt-1 pl-4 lg:pl-6">{errors.password}</p>
                )}
              </div>

              {/* Checkbox */}
              <div className="flex flex-row items-center w-full h-auto gap-4 lg:gap-5">
              ...
              <button
                onClick={handleLogin}
                className={`group w-full h-12 lg:h-14 rounded-xl bg-[#4D4D4D] text-white hover:bg-black font-arialrounded font-normal text-lg lg:text-xl leading-none tracking-wider transition-colors duration-200 ease-in ${dynamicGap}`}
              >
                <span className="inline-block group-hover:animate-trumble">Re-Begin Your Journey</span>
              </button>
            </div>
          </div>
          ...
          <div className={`flex flex-col w-full h-auto ${dynamicGap}`}>
            <div className="flex flex-col lg:flex-row justify-between text-sm w-full gap-2 lg:gap-0">
              <span 
                className="font-arialrounded font-normal text-base lg:text-xl leading-none tracking-wider text-black hover:text-[#737373] cursor-pointer transition-colors duration-300 ease-in"
                onClick={() => navigate("/OrganizerLoginViaSign")}
              >
            ...
            <div className="w-full h-10 lg:h-12 relative flex items-center rounded-xl px-4 lg:px-6 gap-2 font-arialrounded font-normal text-sm lg:text-base leading-none tracking-wider justify-center text-[#737373] bg-white border-2 border-[#737373] group">
              <div className="relative group">
                ...
                <span className="absolute -right-24 lg:-right-52 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-[#FF0000] text-white text-sm px-4 py-2 rounded-md whitespace-nowrap">
                  Coming Soon
                </span>
              </div>
            </div>
